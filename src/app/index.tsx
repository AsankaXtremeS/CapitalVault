...
export default function DailyLedger() {
  ...
  const [isMockPasswordResetVisible, setIsMockPasswordResetVisible] =
    useState(false);
  ...
  const handleForgotPasswordSubmit = async () => {
    ...
    try {
      ...
      if (result === "link_sent") {
        ...
        setAuthSuccess(
          "Demo Account Found! Please set your new password below.",
        );
      }
    } catch (e: any) {
  ...
  const handleMockPasswordResetSubmit = async () => {
    ...
    try {
      await resetPassword(
        ...
        newPasswordForMock.trim(),
      );
      setAuthSuccess(
        "Simulated password reset successfully! You can now log in.",
      );
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                "Success",
                "Your local database was recovered successfully!",
              );
            } catch (e: any) {
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            Alert.alert(
              "Reset Successful",
              "App data is wiped. You are now logged out of a blank app. Log in to recover!",
            );
          },
        },
      ],
  ...
  const [activeSegment, setActiveSegment] = useState<
    "Ledger" | "Calendar" | "Analytics" | "Notes"
  >("Ledger");
  ...
  const getDailyStatsForCalendar = (dayNum: number) => {
    const dayTxs = dailyTxs.filter(
      (tx) => new Date(tx.date).getDate() === dayNum,
    );
    const income = dayTxs
  ...
  const getWeeklyStatsForMonth = (monthIndex: number) => {
    ...
    const monthTxs = yearTxs.filter(
      (tx) => new Date(tx.date).getMonth() === monthIndex,
    );
  ...
  const pickerMonthName = pickerMonth.toLocaleDateString("en-US", {
    month: "long",
    year: "numeric",
  });
      ...
      <View style={styles.container}>
        ...
        <View style={styles.topPeriodHeader}>
          ...
          <View style={styles.headerIcons}>
            ...
            >
              <Search
                color={isSearchActive ? "#0A84FF" : "#8E8E93"}
                size={20}
              />
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.balanceCard}>
            ...
            <View style={styles.balanceCardRow}>
              <View style={styles.balanceCardHalf}>
                ...
                <Text
                  style={[styles.balanceCardValueText, styles.lightBlueText]}
                >
                  +{currencySymbol}{" "}
              ...
              <View style={styles.balanceCardHalf}>
                ...
                <Text
                  style={[styles.balanceCardValueText, styles.lightRedText]}
                >
                  -{currencySymbol}{" "}
        ...
        <View style={styles.segmentedControlRow}>
          {(["Ledger", "Calendar", "Analytics", "Notes"] as const).map(
            (segment) => {
            ...
            {!isSearchActive && Object.keys(groupedTxs).length === 0 && (
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptySubText}>
                  Tap the "+" button below to log ledger entries.
                </Text>
              </View>
            )}
                    ...
                    <View style={styles.txMiddleContainer}>
                      {tx.note ? (
                        <View>
                          ...
                          <Text style={styles.txAccountBelowNote}>
                            {tx.account} •{" "}
                        ...
                        <View style={styles.dayTotals}>
                          {dayIncome > 0 && (
                            <Text style={styles.dayIncomeVal}>
                              +{currencySymbol}{" "}
                              {dayIncome.toLocaleString("en-US", {
                          ...
                          {dayExpense > 0 && (
                            <Text style={styles.dayExpenseVal}>
                              -{currencySymbol}{" "}
                              {dayExpense.toLocaleString("en-US", {
                                maximumFractionDigits: 0,
                              })}
                            </Text>
                          )}
              ...
              {calendarRows.map((row, rowIndex) => (
                <View key={rowIndex} style={styles.gridRow}>
                  {row.map((dayNum, cellIndex) => {
                    if (dayNum === null) {
                      return (
                        <View key={cellIndex} style={styles.gridCellEmpty} />
                      );
                    }

                    const { income, expense, net } =
                      getDailyStatsForCalendar(dayNum);
                    const isToday =
                    ...
                    return (
                      <View
                        key={cellIndex}
                        style={[styles.gridCell, isToday && styles.todayCell]}
                      >
                        <Text
                          style={[
                            styles.dayNumber,
        ...
        {activeSegment === "Analytics" && (
          <ScrollView contentContainerStyle={styles.listContent}>
            ...
            <View style={styles.analyticsStatsGrid}>
              <View
                style={[styles.analyticsStatsCell, { borderColor: "#0A84FF" }]}
              >
              ...
              <View
                style={[styles.analyticsStatsCell, { borderColor: "#FF453A" }]}
              >
            ...
            {getCategoryTotalsList().length === 0 ? (
              <View style={styles.emptyContainer}>
                <Text style={styles.emptyText}>
                  No outlays logged for this period.
                </Text>
              </View>
            ) : (
              getCategoryTotalsList().map((item) => (
                <View key={item.name} style={styles.categoryTotalsCard}>
                  <View style={styles.categoryTotalsMeta}>
                    ...
                    <Text
                      style={[
                        styles.categoryTotalsAmount,
                        item.net >= 0 ? styles.incomeText : styles.expenseText,
                      ]}
                    >
                      {item.net >= 0 ? "+" : ""}
                      {currencySymbol}{" "}
                    ...
                    <View style={styles.categoryTotalsProgressLabelRow}>
                      <Text style={styles.categoryTotalsProgressText}>
                        ...
                        <Text style={styles.incomeText}>
                          {currencySymbol} {item.income.toLocaleString()}
                        </Text>
                      </Text>
                      <Text style={styles.categoryTotalsProgressText}>
                        Out:{" "}
                        <Text style={styles.expenseText}>
                          {currencySymbol} {item.expense.toLocaleString()}
                        </Text>
                      </Text>
                    </View>
                  </View>
            ...
            <View style={styles.notesCard}>
              ...
              <View style={styles.notesFooter}>
                <Text style={styles.notesFooterText}>
                  ✓ Autosaved to local secure storage
                </Text>
              </View>
            </View>
          </ScrollView>
              ...
              <ScrollView contentContainerStyle={styles.modalFormContent}>
                ...
                <View style={styles.typeRow}>
                  <TouchableOpacity
                    style={[
                      styles.typeBtn,
                      txType === "expense" && styles.typeExpenseActive,
                    ]}
                    onPress={() => {
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                      setTxType("expense");
                  ...
                  >
                    <Text
                      style={[
                        styles.typeBtnText,
                        txType === "expense" && styles.typeActiveText,
                      ]}
                    >
                      Expense
                    </Text>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.typeBtn,
                      txType === "income" && styles.typeIncomeActive,
                    ]}
                    onPress={() => {
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                      setTxType("income");
                  ...
                  >
                    <Text
                      style={[
                        styles.typeBtnText,
                        txType === "income" && styles.typeActiveText,
                      ]}
                    >
                      Income
                    </Text>
                ...
                <View style={styles.formItem}>
                  ...
                  >
                    <Text style={styles.amountSelectorVal}>
                        ...
                        {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map(
                          (d, idx) => (
                            <Text
                              key={idx}
                              style={[
                                styles.pickerWeekLabel,
                                idx === 0 && styles.pickerSundayLabel,
                                idx === 6 && styles.pickerSaturdayLabel,
                              ]}
                            >
                              {d}
                            </Text>
                            ...
                            {row.map((dayNum, cellIndex) => {
                              if (dayNum === null) {
                                return (
                                  <View
                                    key={cellIndex}
                                    style={styles.pickerGridCellEmpty}
                                  />
                                );
                              }
                                ...
                                >
                                  <Text
                                    style={[
                                      ...
                                      isSelected &&
                                        styles.pickerDayNumberSelected,
                                      cellIndex === 0 &&
                                        !isSelected &&
                                        styles.pickerSundayText,
                                      cellIndex === 6 &&
                                        !isSelected &&
                                        styles.pickerSaturdayText,
                                    ]}
                                  >
                                    {dayNum}
                                  </Text>
                    ...
                    <View style={styles.gridSelector}>
                      {[
                        ...
                        <TouchableOpacity
                          ...
                          style={[
                            styles.gridItem,
                            category === cat && styles.gridItemActive,
                          ]}
                          onPress={() => {
                            setCategory(cat);
                        ...
                        >
                          <Text
                            style={[
                              styles.gridItemText,
                              category === cat && styles.gridItemTextActive,
                            ]}
                          >
                      ...
                      >
                        <Text style={styles.addCustomGridItemText}>
                          + Custom
                        </Text>
                      </TouchableOpacity>
                    </View>
                  ...
                  <View style={styles.authContainer}>
                    ...
                    {forgotPasswordMode ? (
                      ...
                      <View style={styles.authForm}>
                        ...
                        >
                          <Text style={styles.authSwitchBtnText}>
                            Back to Sign In
                          </Text>
                        </TouchableOpacity>
                      </View>
                  ...
                  <View style={styles.dashboardContainer}>
                    ...
                    <View style={styles.dashboardAccountCard}>
                      ...
                      <View style={styles.accountMeta}>
                        <Text style={styles.accountUserEmail}>
                          {user.email}
                        </Text>
                        <Text style={styles.accountUserId}>
                          Cloud ID: {user.id.substring(0, 15)}...
                        </Text>
                      </View>
                      <TouchableOpacity
                    ...
                    <View style={styles.syncDetailsRow}>
                      ...
                      <Text style={styles.syncDetailsValue}>
                        {lastSyncedAt || "Never"}
                      </Text>
                    </View>
                    ...
                    <View style={styles.settingsSection}>
                      ...
                      <View style={styles.settingSwitchRow}>
                        <View style={styles.settingSwitchMeta}>
                          <Text style={styles.settingSwitchTitle}>
                            Automatic Sync
                          </Text>
                          <Text style={styles.settingSwitchDesc}>
                            Save creations to cloud instantly
                          </Text>
                        </View>
                        <TouchableOpacity
                          style={[
                      ...
                      <View style={styles.settingSwitchRow}>
                        <View style={styles.settingSwitchMeta}>
                          ...
                          <Text style={styles.settingSwitchDesc}>
                            Allow synchronization over cellular
                          </Text>
                        </View>
                        <TouchableOpacity
                          style={[
                      ...
                      <TouchableOpacity
                        ...
                        onPress={async () => {
                          ...
                          try {
                            ...
                            Alert.alert(
                              "Success",
                              "Local vault backed up successfully to the cloud!",
                            );
                          } catch (e: any) {
                            Alert.alert(
                              "Backup Failed",
                              e.message || "Cloud backup failed.",
                            );
                          } finally {
